...
int cameraClass::runFrame(){
    ...
    while(imgLen > 0){
        ...
        // uiLength should include the entire message except TLV header
        pstImgMsg->tlv.uiLength = sizeof(MSG_WLAN_IMAGE_S) - sizeof(TLV_S) + copyLen;
        memcpy(pstImgMsg->aucData, pic->buf + pstImgMsg->uiOffset, copyLen);
        pwlanMsgObj->send(aucTxBuffer, sizeof(TLV_S) + pstImgMsg->tlv.uiLength);
