    ...
    switch(type){
        case SERIAL_MSG_DEVICEINFO_E:
            if(len != sizeof(SERIAL_MSG_CONNECT_S)){
                Serial.printf("Sizeof SERIAL_MSG_CONNECT_S dismatch: rcv: %u, local %u\n", len, sizeof(SERIAL_MSG_CONNECT_S));
            }else{
        ...
        case SERIAL_MSG_WIFI_CONFIG_E:
            if(len != sizeof(SERIAL_MSG_WIFICONFIG_S)){
                Serial.printf("Sizeof SERIAL_MSG_WIFICONFIG_S dismatch: rcv: %u, local %u\n", len, sizeof(SERIAL_MSG_WIFICONFIG_S));
            }else{
